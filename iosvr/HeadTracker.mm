...
namespace iosvr
{
    ...
    void HeadTracker::startTracking()
    {
        ...
        [(CMMotionManager*)_motionManager setAccelerometerUpdateInterval:(1.0/100.0)];
        [(CMMotionManager*)_motionManager startAccelerometerUpdatesToQueue:accelerometerQueue withHandler:^(CMAccelerometerData *accelerometerData, NSError *error)
        {
            ++_sampleCount;
        ...
        [(CMMotionManager*)_motionManager setGyroUpdateInterval:(1.0/100.0)];
        [(CMMotionManager*)_motionManager startGyroUpdatesToQueue:gyroQueue withHandler:^(CMGyroData *gyroData, NSError *error) {
            if (_sampleCount <= kInitialSamplesToSkip) { return; }
        ...
        if ((CMMotionManager*)_motionManager.isDeviceMotionAvailable)
        {
            [(CMMotionManager*)_motionManager startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXArbitraryZVertical];
        ...
        [(CMMotionManager*)_motionManager setDeviceMotionUpdateInterval:(1.0/100.0)];
        [(CMMotionManager*)_motionManager startDeviceMotionUpdatesUsingReferenceFrame:CMAttitudeReferenceFrameXArbitraryZVertical toQueue:deviceMotionQueue withHandler:^(CMDeviceMotion *motion, NSError *error) {
            ++_sampleCount;
            if (_sampleCount <= kInitialSamplesToSkip) { return; }
    ...
    {
      #if HEAD_TRACKER_MODE == HEAD_TRACKER_MODE_EKF
        [(CMMotionManager*)_motionManager stopAccelerometerUpdates];
        [(CMMotionManager*)_motionManager stopGyroUpdates];
      #elif HEAD_TRACKER_MODE == HEAD_TRACKER_MODE_CORE_MOTION || HEAD_TRACKER_MODE == HEAD_TRACKER_MODE_CORE_MOTION_EKF
        [(CMMotionManager*)_motionManager stopDeviceMotionUpdates];
      #endif
    }
    ...
    void HeadTracker::updateDeviceOrientation()
    {
